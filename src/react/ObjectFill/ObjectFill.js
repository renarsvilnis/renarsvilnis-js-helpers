...
export default class ObjectFill extends PureComponent {
  ...
  calcDimensions (props) {
    ...
    const newState = calcFillDimensions(refWidth, refHeight, parseInt(parentStyle.width, 10), parseInt(parentStyle.height, 10), options);
    this.setState(newState);
  }

  render () {
    const style = this.state;
    return cloneElement(this.props.children, {style});
  }
}
